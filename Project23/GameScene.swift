...
//  Project23
//
//  Created by Fayzullayev Nurbek on 18/08/22.
//
//
import AVFoundation
import SpriteKit
...
class GameScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        sequence = [.oneNoBomb, .oneNoBomb, .twoWithOneBomb, .twoWithOneBomb, .three, .one, .chain,.bonus]
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for case let node as SKSpriteNode in nodesAtPoint {
            if node.name == "enemy" {
                ...
                // 7
                let index = activeEnemies.index(of: node)!
   ...
   func createBonusEnemy(){
       var bonusenemy: SKSpriteNode
       bonusenemy = SKSpriteNode()
       bonusenemy.zPosition = 1
       bonusenemy = SKSpriteNode(imageNamed: "bonusEnemy")
      
       
       run(SKAction.playSoundFileNamed("launch.caf", waitForCompletion: false))
       bonusenemy.name = "bonusEnemy"
       
       let randomPosition = CGPoint(x: Int.random(in: 64...960), y: -128)
       bonusenemy.position = randomPosition
       ...
       bonusenemy.physicsBody = SKPhysicsBody(circleOfRadius: 40)
       bonusenemy.physicsBody?.velocity = CGVector(dx: randomXVelocity * 40, dy: randomYVelocity * 40)
       bonusenemy.physicsBody?.angularVelocity = randomAngularVelocity
       bonusenemy.physicsBody?.collisionBitMask = 0

       addChild(bonusenemy)

       activeEnemies.append(bonusenemy)
    }
    ...
    func tossEnemies() {
        ...
        case .bonus:
            createBonusEnemy()
           // DispatchQueue.main.asyncAfter(deadline: .now() + (chainDelay/20.0)) {[unowned self] in self.createBonusEnemy()}
        }
    ...
    func endGame(triggeredByBomb: Bool) {
        ...
        gameover.size = CGSize(width: 1024, height: 768)
      
        run(SKAction.playSoundFileNamed("gameOverRecord.m4a", waitForCompletion: false))
